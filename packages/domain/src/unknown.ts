 ...
 * total reads — an object to look a field up in, and a list to walk.
 *
 * It sits below the modules that read replies (the transcript, the usage report, the runtime
 * events) because they all need it and it needs nothing: a leaf, so no read can close a loop, and
 * one place, so a guard that changes changes for every reader at once.
 */
...
/**
 * An object to look a field up in, from a value that is `unknown` because it came off the wire. The
 * protocol promises a shape and a reply is still unknown until something reads a field off it, so
 * the read is total rather than a cast.
 */
