/**
 * The interface's words, in English. The Chinese dictionary answers the same keys, and the
 * compiler checks that it answers every one of them.
 */

export const EN = {
  'window.minimize': 'Minimize window',
  'window.maximize': 'Maximize window',
  'window.restore': 'Restore window',
  'window.close': 'Close window',
  'tasks.title': 'Tasks',
  'tasks.intro': 'A task runs its prompt in a folder on a schedule, while the workbench is open.',
  'tasks.empty': 'No tasks yet.',
  'tasks.emptyBody': 'A task is a prompt that runs on its own: a nightly check, a morning summary.',
  'tasks.new': 'New task',
  'tasks.name': 'Name',
  'tasks.nameHint': 'Also the title of every conversation a run creates.',
  'tasks.prompt': 'What to ask',
  'tasks.promptHint': 'The message each run starts with.',
  'tasks.folder': 'Folder',
  'tasks.level': 'Permission level',
  'tasks.schedule': 'Schedule',
  'tasks.invalidInterval': 'Use at least {count} whole minutes.',
  'tasks.everyMinutes': 'Every {count} minutes',
  'tasks.dailyAt': 'Every day at {time}',
  'tasks.nextRun': 'Next run {when}',
  'tasks.save': 'Save task',
  'tasks.cancel': 'Cancel',
  'tasks.runNow': 'Run now',
  'tasks.stop': 'Stop this task',
  'tasks.start': 'Start this task',
  'tasks.stopped': 'Stopped',
  'tasks.delete': 'Delete task',
  'tasks.history': 'Runs',
  'tasks.noRuns': 'It has not run yet.',
  'tasks.runOk': 'Finished',
  'tasks.runFailed': 'Failed',
  'tasks.runSkipped': 'Skipped',
  'tasks.runRunning': 'Running',
  'tasks.runCatchUp': 'caught up',
  'tasks.runRefusals': '{count} refused',
  'tasks.skipMissingFolder': 'the folder is not there',
  'tasks.levelAsk': 'Steps that need approval are refused while nobody is watching.',
  'tasks.levelFull': 'It will do anything while nobody is watching.',
  'tasks.openRun': 'Open the conversation from {when}',
  'tasks.open': 'Open',
  'tasks.minutes': 'Minutes',
  'tasks.time': 'At',
  'tasks.kindEvery': 'Interval',
  'tasks.kindDaily': 'Daily',
  'settings.open': 'Settings',
  'sidebar.places': 'Places',
  'sidebar.newConversation': 'New conversation',
  'sidebar.newConversationIn': 'in {folder}',
  'sidebar.newConversationNowhere': 'in no folder yet',
  'sidebar.addFolder': 'Add a folder',
  'sidebar.search': 'Search',
  'sidebar.folders': 'Folders',
  'sidebar.noFolders': 'No folders yet.',
  'sidebar.noConversations': 'No conversations yet',
  'sidebar.conversations': '{count} conversations',
  'sidebar.oneConversation': '1 conversation',
  'sidebar.startIn': 'Start a conversation in {folder}',
  'sidebar.expand': 'Expand {folder}',
  'sidebar.collapse': 'Collapse {folder}',
  'sidebar.actions': 'Actions for {title}',
  'sidebar.tasks': 'Tasks',
  'sidebar.expandTasks': 'Expand the tasks in {folder}',
  'sidebar.collapseTasks': 'Collapse the tasks in {folder}',
  'sidebar.runNow': 'Run {name} now',
  'sidebar.openTask': 'Open {name}',
  'sidebar.renameAction': 'Rename',
  'sidebar.conversationTitle': 'Conversation title',
  'sidebar.deleteAction': 'Delete',
  'sidebar.exportAction': 'Export',
  'sidebar.exportedTo': 'Exported to {path}',
  'sidebar.archiveAction': 'Archive',
  'sidebar.unarchiveAction': 'Unarchive',
  'sidebar.archiveBlocked': 'Stop it first',
  'sidebar.archived': 'Archived',
  'sidebar.archivedHint': 'Put away. Sending a message brings it back.',
  'sidebar.expandArchived': 'Expand the archived conversations',
  'sidebar.collapseArchived': 'Collapse the archived conversations',
  'sidebar.more': 'and {count} more',
  'sidebar.showAll': 'Show all {count}',
  'sidebar.showFewer': 'Show fewer',
  'sidebar.idle': 'idle',
  'sidebar.working': 'working',
  'sidebar.waiting': 'waiting for you',
  'sidebar.browserNoPicker': 'A folder can only be added in the desktop window.',
  'empty.noFolder.title': 'Add a folder to begin',
  'empty.noFolder.body':
    'A folder is what the agent reads and edits. Everything you ask for happens inside one, and nothing happens outside it without your say-so.',
  'empty.chooseFolder': 'Choose a folder',
  'empty.greetingMorning': 'Good morning',
  'empty.greetingAfternoon': 'Good afternoon',
  'empty.greetingEvening': 'Good evening',
  'empty.startFix': 'Fix the last error',
  'empty.startReview': 'Review what changed',
  'empty.startTests': 'Write tests',
  'empty.startExplain': 'Explain this folder',
  'conversation.opening': 'Opening conversation…',
  'conversation.openFailed': 'Could not open this conversation',
  'conversation.retry': 'Retry',
  'composer.folderSwitch': 'Change the folder: {folder}',
  'composer.folders': 'Folders',
  'composer.chooseFolder': 'Choose a folder…',
  'composer.messageLabel': 'Message the agent',
  'composer.placeholder': 'Ask the agent to change something…',
  'composer.placeholderNoFolder': 'Add a folder first',
  'composer.send': 'Send',
  'composer.stop': 'Stop',
  'composer.queue': 'Queue',
  'composer.steer': 'Steer',
  'composer.thinking': 'Thinking effort',
  'composer.cancelQueued': 'Cancel the queued message: {text}',
  'composer.queued': 'Queued',
  'composer.queuedList': 'Queued messages',
  'composer.cancel': 'Cancel',
  'composer.steered': 'Steered',
  'composer.editQueued': 'Edit the queued message: {text}',
  'composer.deleteQueued': 'Delete the queued message: {text}',
  'composer.edit': 'Edit',
  'composer.delete': 'Delete',
  'composer.queueStopped': 'The queue is stopped.',
  'composer.queueStoppedWhy': 'A failed turn and Stop both stop it; Resume sends what is waiting.',
  'composer.resume': 'Resume',
  'level.plan': 'Plan',
  'level.plan.about': 'Reads only. The agent proposes changes instead of making them.',
  'level.ask': 'Ask',
  'level.ask.about': 'Asks before every file change and every command.',
  'level.acceptEdits': 'Accept edits',
  'level.acceptEdits.about': 'File changes run without asking. Commands still ask.',
  'level.fullAccess': 'Full access',
  'level.fullAccess.about': 'Nothing asks. Every tool call runs immediately.',
  'level.chipTitle': 'Permission level',
  'level.chipTitleHere': 'This conversation: {description}',
  'risk.reads': 'Reads',
  'risk.writes': 'Writes',
  'risk.runs': 'Runs commands',
  'thinking.off': 'Off',
  'thinking.minimal': 'Minimal',
  'thinking.low': 'Low',
  'thinking.medium': 'Medium',
  'thinking.high': 'High',
  'thinking.xhigh': 'Extra high',
  'thinking.max': 'Max',
  'rule.conversation': 'This conversation',
  'rule.workspace': 'This workspace',
  'approval.inFolder': 'in {path}',
  'approval.region': 'Waiting for your decision',
  'approval.change': 'Wants to change a file',
  'approval.command': 'Wants to run a command',
  'approval.allowOnce': 'Allow once',
  'approval.alwaysAllow': 'Always allow',
  'approval.deny': 'Deny',
  'approval.rememberFor': 'Remember this for',
  'approval.scopeConversation': 'this conversation',
  'approval.scopeWorkspace': 'this workspace',
  'approval.reasonLabel': 'Reason for denying',
  'approval.reasonPlaceholder': 'Reason (optional)',
  'palette.dialog': 'Switch conversation',
  'palette.search': 'Search conversations',
  'palette.placeholder': 'Go to a conversation…',
  'palette.list': 'Conversations',
  'palette.empty': 'No conversations yet.',
  'palette.noMatch': 'Nothing matches that.',
  'unlock.title': 'This workbench is not yours yet',
  'unlock.body':
    'Alpha is running on another machine, and it can run shell commands in a folder there. Paste the access token from that machine, from Settings under Browser access.',
  'unlock.token': 'Access token',
  'unlock.tokenPlaceholder': 'paste the token',
  'unlock.submit': 'Open the workbench',
  'unlock.opening': 'Opening…',
  'header.tokens': 'Tokens and cost for this conversation',
  'header.updated': 'updated {age}',
  'model.chipTitleHere': 'This conversation runs on {model}. Click to change it.',
  'model.chipTitleDefault': 'New conversations start on {model}. Click to change it.',
  'model.menuLabel': 'The model this runs on',
  'model.none': 'No model',
  'model.noneHint': 'Choose a model here, or add one in Settings.',
  'model.configureHint': 'Open Providers settings to add a model.',
  'message.copy': 'Copy',
  'message.copied': 'Copied',
  'message.copyFailed': 'Copy failed',
  'message.copyMessage': 'Copy message',
  'message.copyAnswer': 'Copy answer',
  'message.copyOutput': 'Copy the output of {tool}',
  'message.copyOutputLabel': 'Copy output',
  'message.edit': 'Edit',
  'message.editLabel': 'Edit the message',
  'message.attachment': 'Attached image',
  'message.transcript': 'The conversation',
  'changes.views': 'Conversation views',
  'changes.conversation': 'Conversation',
  'changes.tab': 'Changes',
  'changes.none': 'No runs to review.',
  'changes.noFiles': 'No net file changes.',
  'changes.oneFile': '1 file',
  'changes.files': '{count} files',
  'changes.recovered': 'Recovered after restart',
  'changes.incomplete': 'Some paths were not scanned. This list may be incomplete.',
  'changes.added': 'Added',
  'changes.modified': 'Modified',
  'changes.deleted': 'Deleted',
  'changes.before': 'Before',
  'changes.after': 'After',
  'changes.noPreview': 'Preview unavailable',
  'mcp.tab': 'MCP requests',
  'mcp.formTitle': 'Server question',
  'mcp.from': '{server} via {tool}',
  'mcp.required': 'Required',
  'mcp.noSelection': 'No selection',
  'mcp.send': 'Send answer',
  'mcp.decline': 'Decline',
  'mcp.cancel': 'Cancel',
  'mcp.invalidAnswer': 'Check the fields and try again.',
  'mcp.sharedContent': 'Shared with server',
  'mcp.methodElicitation': 'Question',
  'mcp.methodSampling': 'Model request',
  'mcp.pending': 'Waiting',
  'mcp.accepted': 'Accepted',
  'mcp.declined': 'Declined',
  'mcp.cancelled': 'Cancelled',
  'mcp.refused': 'Refused',
  'mcp.interrupted': 'Interrupted',
  'mcp.samplingTitle': 'Model request',
  'mcp.samplingModel': '{provider} / {model}',
  'mcp.samplingLimit': 'Up to {tokens} output tokens',
  'mcp.samplingContext': 'The server requested context. Only the text shown here will be sent to the model.',
  'mcp.samplingHints': 'Server preference: {hints}',
  'mcp.samplingSystem': 'System prompt',
  'mcp.samplingUser': 'User message',
  'mcp.samplingAssistant': 'Assistant message',
  'mcp.samplingGenerate': 'Generate',
  'mcp.samplingGenerating': 'Generating response…',
  'mcp.samplingReview': 'Review response',
  'mcp.samplingUsage': '{tokens} tokens used',
  'mcp.samplingShare': 'Share with server',
  'mcp.samplingDiscard': 'Discard response',
  'mcp.samplingFailed': 'Could not complete this request.',
  'mcp.samplingSubmitted': 'Prompt sent to model',
  'mcp.samplingShared': 'Response shared with server',
  'composer.attach': 'Attach a picture',
  'composer.attached': 'Attached pictures',
  'composer.removeAttachment': 'Remove {name}',
  'composer.attachmentRefused': 'Only pictures up to 4 MB can be attached.',
  'message.regenerate': 'Regenerate',
  'message.resend': 'Resend, replacing what followed',
  'message.fork': 'Fork into a new conversation',
  'message.editNote':
    'Replacing drops the messages after this one. Forking copies them to a new conversation and leaves this one alone.',
  'message.stopped': 'Stopped — what arrived before the stop is kept',
  // A run that failed without saying anything: the sentence is the interface's, so it is here and
  // in the window's language. What a failure did say is the provider's own words, quoted as they
  // came (ADR-0010), and travels with the event instead.
  'message.failed': 'The turn failed.',
  // A turn that never started, by which of Alpha's own refusals it was stopped: main answers the
  // case and the sentences are here, in the window's language, because main is not the thing with
  // a language (ADR-0010). Provider words never appear in these: they are quoted elsewhere.
  'refusal.noModel': 'No model is configured for this conversation. Choose one in the composer or add one in Settings.',
  'refusal.modelNotServed': 'Alpha has no model {model} on {provider} to run this conversation on.',
  'refusal.noProvider': 'Alpha has no provider called {provider}.',
  'refusal.keyUnreadable': 'Alpha could not read the key for {provider}. Enter it again under Settings, Providers.',
  'refusal.noKey': 'Alpha has no key for {provider}. Add one under Settings, Providers.',
  'refusal.pictures': '{model} does not take pictures. Turn that on for it under Models in Settings.',
  'message.thinking': 'Thinking',
  'message.compacted': 'History summarised here',
  'message.compactedCount': 'History summarised here · {count} messages',
  'message.turn': 'Turn · {tokens} tokens',
  'message.turnEarlier': 'Earlier turns · {tokens} tokens',
  'message.turnCost': 'Turn · {tokens} tokens · {cost}',
  'message.turnEarlierCost': 'Earlier turns · {tokens} tokens · {cost}',
  'tool.markRule': 'rule',
  'tool.markOnce': 'allowed once',
  'tool.markAlways': 'always allowed',
  'tool.markDenied': 'denied',
  'tool.markBlocked': 'blocked',
  'tool.running': 'running',
  'tool.done': 'done',
  'tool.failed': 'failed',
  'tool.arguments': 'Arguments',
  'tool.output': 'Output',
  'tool.gate': 'Gate',
  'tool.diff': 'Diff',
  'tool.truncated': 'Truncated. Full output: {path}',
  'gate.auto': 'Allowed automatically by the {level} level.',
  'gate.once': 'Allowed once by you, at the {level} level.',
  'gate.always': 'Allowed by a new rule, at the {level} level.',
  'gate.denied': 'Denied by you, at the {level} level.',
  'gate.blocked': 'Blocked by the {level} level.',
  'gate.allowedByRule': 'Allowed by a remembered rule, at the {level} level.',
  'settings.title': 'Settings',
  'settings.sections': 'Settings sections',
  'settings.back': 'Back to the workbench',
  'settings.groupAgent': 'The agent',
  'settings.groupApp': 'The workbench',
  'settings.tabProviders': 'Providers',
  'settings.tabPermissions': 'Permissions',
  'settings.tabAppearance': 'Appearance',
  'settings.tabBrowserAccess': 'Browser access',

  'settings.providersNote':
    'Where the workbench may reach, and with which key. Keys are held in the OS keychain and handed only to the model runtime, for the requests it makes itself.',
  'settings.permissionsNote': 'What the agent may do on its own, and what it has to ask about.',
  'settings.appearanceNote': 'Which palette the workbench is drawn in, and in which colour.',
  'settings.browserAccessNote':
    'Serve this workbench to a browser on another device. Whoever holds the token can read every conversation and answer every approval — it is a remote control for this machine, not a viewer.',
  'settings.browserHost':
    'You are reading this in a browser, where the switch is the machine to hold: a browser cannot add a folder either.',
  'settings.serve': 'Serve to a browser',
  'settings.serveLabel': 'Serve this workbench to a browser',
  'settings.whoCanReach': 'Who can reach it',
  'settings.bindLocal': 'This machine only',
  'settings.bindNetwork': 'Anything on this network',
  'settings.port': 'Port',
  'settings.openAt': 'Open it at',
  'settings.portHint': '0 picks one',
  'settings.replaceToken': 'Replace',
  'settings.tokenMinted': 'minted when you switch it on',
  'settings.addProvider': 'Add a provider',
  'settings.apiOpenai': 'OpenAI Chat Completions',
  'settings.apiAnthropic': 'Anthropic Messages',
  'settings.apiResponses': 'OpenAI Responses',
  'settings.apiOpenaiNote':
    'OpenAI itself, and the many services that copy it: most gateways, and anything running locally. This is the one to try first. The request goes to /chat/completions under the base url, so the base url is where the version segment goes — /v1 for most of them.',
  'settings.apiAnthropicNote':
    'The Claude API, and the proxies that speak it. The request goes to /v1/messages under the base url, and the client adds that /v1 itself: give the base url one of its own and the call reaches /v1/v1/messages.',
  'settings.apiResponsesNote':
    'The newer OpenAI shape: the request goes to /responses under the base url, which carries the version segment (/v1 for most of them). Whoever copied that shape works here too.',
  'settings.noKeychain':
    'This system offers no keychain, so keys are stored in plain text in the workbench data folder.',
  'settings.noProviders': 'No providers yet.',
  'settings.keyStored': 'key stored',
  'settings.noKey': 'no key',
  'settings.oneModel': '1 model',
  'settings.models': '{count} models',
  'settings.apiKeyFor': 'API key for {provider}',
  'settings.pasteKey': 'Paste the key',
  'settings.saveKey': 'Save key',
  'settings.test': 'Test',
  'settings.testTimeout': 'The provider did not answer within {seconds} seconds.',
  'settings.testRequestFailed': 'The provider did not answer.',
  'settings.testRequestFailedSaid': 'The provider did not answer: {said}',
  'settings.testProviderError': 'The provider did not answer: {said}',
  'settings.testEmptyAnswer': 'The provider answered with nothing.',
  'settings.removeProvider': 'Delete',
  'settings.fieldId': 'Id',
  'settings.fieldName': 'Name',
  'settings.fieldBaseUrl': 'Base URL',
  'settings.fieldIdPlaceholder': 'local-llama',
  'settings.fieldNamePlaceholder': 'Local Llama',
  'settings.fieldBaseUrlPlaceholder': 'the provider base url',
  'settings.noModels': 'No models yet. Add the ones this provider serves.',
  'settings.defaultModel': 'New conversations start on',
  'settings.defaultModelAuto': 'The first model there is',
  'settings.defaultModelNote':
    'Falls back to the first model of the first provider that has one. A conversation can change its own model from the chip at the foot of its composer at any time.',
  'settings.saveModels': 'Save models',
  'settings.reset': 'Reset',
  'settings.reasoning': 'Thinks before answering',
  'settings.takesPictures': 'Takes pictures',
  'settings.addModel': 'Add model',
  'settings.removeModel': 'Remove',
  'settings.displayNamePlaceholder': 'What it is called',
  'settings.modelsLabel': 'Models',
  'settings.modelRow': 'Model {index}',
  'settings.modelId': 'Model id',
  'settings.modelName': 'Display name',
  'settings.modelContext': 'Context window',
  'settings.modelMaxTokens': 'Max output',
  'settings.trackCost': 'Track cost',
  'settings.priceUnit': 'USD per 1M tokens',
  'settings.priceInput': 'Input',
  'settings.priceOutput': 'Output',
  'settings.priceCacheRead': 'Cache read',
  'settings.priceCacheWrite': 'Cache write',
  'settings.wireProtocol': 'Wire protocol',
  'settings.authStyle': 'API key sent as',
  'settings.authStyleApiKey': 'The header the wire uses',
  'settings.authStyleBearer': 'Bearer token (Authorization)',
  'settings.authStyleNote':
    'Where the key rides. The default is the header the wire uses — x-api-key on the Anthropic wire, Authorization: Bearer on the OpenAI-shaped ones. Pick Bearer only for a provider that refuses that default.',
  'settings.defaultLevel': 'Default permission level',
  'settings.defaultLevelNote':
    'New conversations in this workspace start here. An open conversation keeps its own level — change that at the foot of its composer.',
  'settings.remembered': 'Remembered approvals',
  'settings.remembered.none': 'Nothing is remembered yet. "Always allow" on a card adds a rule here.',
  'settings.remembered.some': 'These calls run without asking. Revoking one makes the next matching call ask again.',
  'settings.revoke': 'Revoke the {tool} rule for {pattern}',
  'settings.revokeAction': 'Revoke',
  'settings.anyPattern': '(anything)',
  // MCP servers: the connections the agent reaches extra tools through. A server is configured here
  // rather than in a file, and what the page cannot show — whether the server is there — it says.
  'settings.tabMcp': 'MCP servers',
  'settings.mcpNote':
    'Servers the agent reaches for tools. A server is a command this machine runs, or an endpoint Alpha posts to; what it offers becomes the agent’s tools, and every one of those goes through the permission gate like any other. What a server is reached with — its environment, or its headers — is stored in plain text in mcp.json, which is where this page writes them.',
  'settings.mcpEmpty': 'No MCP servers yet. Add the one you want the agent to reach.',
  'settings.addMcpServer': 'Add an MCP server',
  'settings.mcpToolsOne': '1 tool',
  'settings.mcpTools': '{count} tools',
  'settings.mcpStarting': 'Reaching it…',
  'settings.mcpUnreachable': 'Not reached',
  'settings.mcpReconnect': 'Reconnect',
  'settings.mcpRemove': 'Delete',
  'settings.mcpRemoveNamed': 'Delete the {name} server',
  'settings.mcpName': 'Name',
  'settings.mcpNamePlaceholder': 'files',
  'settings.mcpNameNote': 'An id: lower-case letters, digits and dashes. Every tool it offers is named after it.',
  'settings.mcpNameTaken': 'A server with this name is already configured.',
  'settings.mcpKind': 'How it is reached',
  'settings.mcpKindCommand': 'A command this machine runs',
  'settings.mcpKindUrl': 'An endpoint Alpha posts to',
  'settings.mcpCommand': 'Command',
  'settings.mcpCommandPlaceholder': 'npx',
  'settings.mcpArgs': 'Arguments',
  'settings.mcpArgsPlaceholder': 'one per line',
  'settings.mcpEnv': 'Environment',
  'settings.mcpUrl': 'URL',
  'settings.mcpUrlPlaceholder': 'the endpoint address',
  'settings.mcpHeaders': 'Headers',
  'settings.mcpPairsPlaceholder': 'NAME=value, one per line',
  'settings.mcpSave': 'Save',
  'settings.theme': 'Theme',
  'settings.themeNote':
    'Light is what a new workbench opens on. The dark palette is its own set of colours rather than an inversion, and following the system switches between the two as the machine does.',
  'settings.themeSystem': 'Follow the system',
  'settings.themeDark': 'Dark',
  'settings.themeLight': 'Light',
  'settings.language': 'Language',
  'settings.languageNote':
    'Which language this interface is written in. What the agent is told stays in the language of the conversation itself, so translating the interface does not change how a conversation reads.',

  // The rail's fold, named the way it will move (C5.4): one name for each direction. On a phone the
  // same control moves the settings menu, which is the column settings keeps, and that name says so.
  'sidebar.hideRail': 'Hide the rail',
  'sidebar.showRail': 'Show the rail',
  'settings.hideMenu': 'Hide the settings menu',
  'settings.showMenu': 'Show the settings menu',
  // What the session has spent, said once — as the tooltip on the view head's title (C5.4).
  'header.spend': 'Tokens for this conversation: {tokens}',
  'header.spendCost': 'Tokens for this conversation: {tokens} · {cost}',
} as const
