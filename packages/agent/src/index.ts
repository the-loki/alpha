/**
 * The agent, as Alpha assembles it: the plugin contract bound to pi (`AlphaPlugin`), the assembly
 * that turns plugins into a running `Agent`, and the `afterRun` driver that waits a run out and
 * decides whether it is over. The faces themselves are `@alpha/plugin` — pure, so a capability's
