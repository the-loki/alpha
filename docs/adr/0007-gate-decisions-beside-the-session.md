...
`decisions/<conversationId>.json` under the app's data directory, and merged back into the
transcript when it is read from the session.

## Context

ADR-0004 makes the pi session the record of the transcript, and pi's entries carry the tool call,
its arguments, its result and its timing — everything the runtime observed. They do not carry the
decision, because the decision is not pi's: it is made by Alpha's gate, from Alpha's permission
level, Alpha's remembered rules, and the user's answer to a card that pi never saw.
...
- **Write the decision into the session as a custom entry.** pi supports custom entries, but they
  would sit in a format Alpha does not own, would need a migration story of their own, and would
  make an Alpha session mean something different to any other pi-based tool reading it.
...
- **A file per conversation, written by Alpha** (chosen). One small JSON object keyed by call id,
  ...
  conversation. The session stays a pure pi artifact; the note stays Alpha's.
