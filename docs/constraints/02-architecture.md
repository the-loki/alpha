...
C2.1). They are the two that build *on* the agent: `@alpha/agent`, the mechanism half of the base —
the history a run starts from (a session's entries folded into pi's messages), the plugin contract
bound to pi, the assembly, the `afterRun` driver; and `@alpha/internal-plugins`, Alpha's own
plugins, whose faces are where pi's own shapes are the capability (a tool is an `AgentTool`, a hook
is asked about pi's run), which is
[C2.8](#c28--a-capability-is-a-plugin-not-a-branch-in-the-runtime)'s exception carrying its own
adapter. The list is written down rather than inferred, so the exception stays countable: two
packages, and a package that wants in asks by adding itself to the list and to the table.
...
apps/desktop/src/          the app: one package, three processes
  ...
  renderer/  ──┘                    ├──> @alpha/contract     dictionary, the wire between
                          ...
                          └──> @alpha/internal-plugins ──> @alpha/agent, @alpha/gate, the agent library
```
...
sits on the pure faces, and the built-in plugins sit on the gate, the agent base and the agent library
...
| `@alpha/internal-plugins` | Alpha's own plugins, one file each: the gate on `beforeToolCall`, auto-retry on `afterRun`, the four tools, and compaction | `@alpha/agent`, `@alpha/domain`, `@alpha/gate`, `@alpha/plugin`, `@alpha/sessions`, the agent library |
...
`beforeToolCall`, auto-retry on `afterRun`, the four tools, compaction. The plugin is the unit —
...
face hung on it. Nothing is copied either way: the plugin holds the ladder's ports, and the runtime
...
window, the seams Electron owns. The base those run on — the assembly, the `afterRun` driver, the
plugin contract bound to pi — is `@alpha/agent`, a package rather than a folder in `main/runtime`:
it is what capabilities and their tests stand on, and a capability that has to ask `main` for a way
to run an agent has not been assembled, it has been wired.
